...
import { MAX_TOOL_SNAPSHOT_BYTES, type CanvasSnapshot, type ElementPreview } from "./protocol.js";
...
export function createCanvasSnapshot(projection: Projection): CanvasSnapshot | undefined {
  const snapshotJson = JSON.stringify(projection);
  if (Buffer.byteLength(snapshotJson, "utf8") > MAX_TOOL_SNAPSHOT_BYTES) {
    return undefined;
  }
  return { projection: structuredClone(projection) };
}

export function createElementPreviews(
  document: Projection["document"],
  elementIds: string[],
): { elements: ElementPreview[]; missingIds: string[] } {
  const locations = inspectElements(document, elementIds);
  const foundIds = new Set(locations.map(({ element }) => element.id));
  const elements = locations.map(({ page, element }): ElementPreview => {
